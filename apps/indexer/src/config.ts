...
export const config = {
  ...
  // Default: Haiku 4.5 — the brain's judgements are short structured-JSON calls, so the fast/cheap
  // tier fits. (The old claude-3.5-sonnet slug was retired by OpenRouter and 404s — Jul 2026.)
  openrouterApiKey: (process.env.OPENROUTER_API_KEY || '').trim(),
  openrouterModel: (process.env.OPENROUTER_MODEL || 'anthropic/claude-haiku-4.5').trim(),
