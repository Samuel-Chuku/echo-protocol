  ...
  Query: {
    markets: async (_: unknown, a: { mode?: number; status?: string; requester?: string; openOnly?: boolean; limit?: number }) => {
      ...
      const rows = await db.select().from(markets)
        ...
        .orderBy(desc(markets.id)).limit(a.limit ?? 100).all();
      return rows.map(marketOut);
    },
    market: async (_: unknown, a: { id: number }) => {
      const m = await db.select().from(markets).where(eq(markets.id, a.id)).get();
      return m ? marketOut(m) : null;
    },
    marketApplications: (_: unknown, a: { marketId: number }) =>
      db.select().from(applications).where(eq(applications.marketId, a.marketId)).all(),
    applications: (_: unknown, a: { participant: string }) =>
      db.select().from(applications).where(eq(applications.participant, a.participant)).orderBy(desc(applications.createdAt)).all(),
    findings: (_: unknown, a: { marketId: number }) =>
      db.select().from(findings).where(eq(findings.marketId, a.marketId)).orderBy(findings.idx).all(),
    milestones: (_: unknown, a: { marketId: number }) =>
      db.select().from(milestones).where(eq(milestones.marketId, a.marketId)).orderBy(milestones.idx).all(),
    ...
    activity: async (_: unknown, a: { address: string; status?: string; limit?: number }) => {
      ...
      const mine = await db.select({ id: markets.id }).from(markets).where(eq(markets.requester, a.address)).all();
      ...
      const rows = await db.select().from(events).where(ownership)
        .orderBy(desc(events.blockNumber), desc(events.logIndex)).limit(a.limit ?? 100).all();
      return rows
    ...
    disputes: (_: unknown, a: { status?: number }) =>
      db.select().from(disputes)
        ...
        .orderBy(desc(disputes.id)).all(),

    health: async () => {
      const cur = await db.select().from(cursor).where(eq(cursor.id, 'head')).get();
      ...
      const mCount = await db.select({ c: sql<number>`count(*)` }).from(markets).get();
      const eCount = await db.select({ c: sql<number>`count(*)` }).from(events).get();
      return {
