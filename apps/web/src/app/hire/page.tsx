...
import { Card, Field, Badge, Button, EmptyState, CARD_CLASS, ProgressSteps, TierTrack, type TierStep } from '@/components/ui';
 ...
 * then manage what you've created from /hire/[id]. Each create auto-approves the exact USDC it needs.
 */
export default function HirePage() {
  ...
  return (
    <div>
      ...
      <IdentityBanner />
      <CreateMarket sdk={sdk} account={account} agentId={agentId} feeBps={feeBps} />
...
function CreateMarket({ sdk, account, agentId, feeBps }: { sdk: ReturnType<typeof useEcho>['sdk']; account?: `0x${string}`; agentId: string; feeBps?: bigint }) {
  ...
  return (
    <section className="mb-8">
      <button onClick={() => setType(null)} className="text-xs text-white/40 hover:text-white transition mb-3">← pick a different type</button>
      {type === EchoMode.OpenMarket && <OpenWizard sdk={sdk} account={account} agentId={agentId} feeBps={feeBps} />}
      {type === EchoMode.DirectJob && <DirectWizard sdk={sdk} account={account} agentId={agentId} feeBps={feeBps} />}
      {type === EchoMode.Bounty && <BountyWizard sdk={sdk} account={account} agentId={agentId} feeBps={feeBps} />}
    </section>
  );
}

type WizardProps = { sdk: ReturnType<typeof useEcho>['sdk']; account?: `0x${string}`; agentId: string; feeBps?: bigint };
...
function OpenWizard({ sdk, account, agentId, feeBps }: WizardProps) {
    ...
    <div className={`${CARD_CLASS} max-w-2xl`}>
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 text-sm space-y-1.5">
            <div className="flex justify-between"><span className="text-white/50">Escrow to stake</span><span className="font-mono text-white">${escrow} USDC</span></div>
            <div className="flex justify-between"><span className="text-white/50">Platform fee (on payouts)</span><span className="font-mono text-white/70">${feeOn(escrow, feeBps)} USDC est.</span></div>
      ...
      {deployOpen && (
        <TxModal
          ...
          description={`Approves $${escrow} USDC to the market, then creates it.`}
...
function DirectWizard({ sdk, account, agentId, feeBps }: WizardProps) {
  const [step, setStep] = useState(0);
  const [subject, setSubject] = useState('');
  const [desc, setDesc] = useState('');
  ...
  return (
    <div className={`${CARD_CLASS} max-w-2xl`}>
      ...
      {step === 1 && (
        <div className="space-y-3">
          <Field label="milestone amounts (USDC, comma separated)" value={milestones} onChange={(e) => setMilestones(e.target.value)} />
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 text-sm space-y-1.5">
            <div className="flex justify-between"><span className="text-white/50">Worker</span><span className="font-mono text-white">{worker}</span></div>
...
function BountyWizard({ sdk, account, agentId, feeBps }: WizardProps) {
  const [step, setStep] = useState(0);
  const [subject, setSubject] = useState('');
  const [desc, setDesc] = useState('');
      ...
      {step === 1 && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-1">
            ...
            <Field label="ghost deadline (days)" value={ghostDays} onChange={(e) => setGhostDays(e.target.value)} />
            <Field label="required proofs" value={requiredProofs} onChange={(e) => setProofs(e.target.value)} />
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => setStep(0)}>Back</Button>
      ...
      <div className={CARD_CLASS}>
        ...
        {isConnected && !fetching && !error && rows.length === 0 && (
          <EmptyState
            ...
            desc="Markets need a minimum of $500 USDC to fund, plus a 2% platform fee on payouts. Pick a type above to get started."
          />
        )}
        {rows.length > 0 && (
