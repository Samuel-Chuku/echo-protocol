...
/**
 * Activity feed for the connected wallet, read from the indexer (#2, #10). Filters to PENDING (waiting
 * on an action) or COMPLETED. Replaces the old landing's raw getLogs scan — that was the source of the
...
export default function ActivityPage() {
  ...
  const [{ data, fetching, error }] = useQuery<{ activity: ActivityRow[] }>({
    ...
    variables: { address: address ?? '', status: filter === 'ALL' ? undefined : filter, limit: 100 },
      ...
      <div className="sm:col-span-2">
        {!isConnected ? (
          ...
          <Card title="Your activity">
            ...
            {!fetching && !error && rows.length === 0 && <p className="text-xs text-gray-400">No activity yet.</p>}

            {rows.length > 0 && (
              <ul className="divide-y divide-gray-100">
                {rows.map((r) => (
                  <li key={r.id} className="flex items-center gap-3 py-2">
                    <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${r.state === 'PENDING' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-500'}`}>
